        ...
        for opt_level in optimizations:
            ...
            for decompiler in decompilers:
                ...
                for dec_file in dec_files:
                    ...
                    src_path = os.path.join(src_dir, f"{dec_file.split('.')[0]}.c")
                    dec_src = dec_vs_src(dec_path, src_path, ['func_1'])
                    dec_ir = dec_vs_ir(dec_path, ir_path, ['func_1'])
        ...
        for opt_level in optimizations:
            ...
            for decompiler in decompilers:
                ...
                for dec_file in dec_files:
                    ...
                    for func in dec_src:
                        if func in dec_src and func in dec_ir:
                            log_line = f"{dec_file}\t{dec_src[func]}\t{dec_ir[func]}"
                        elif func in dec_src and func not in dec_ir:
                            log_line = f"{dec_file}\t{dec_src[func]}\t0"
                        elif func not in dec_src and func in dec_ir:
                            log_line = f"{dec_file}\t0\t{dec_ir[func]}"
                        else:
                            log_line = f"{dec_file}\t0\t0"
                    logs.append(log_line)
