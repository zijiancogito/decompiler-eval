            ...
            for decompiler in decompilers:
                ...
                for dec_file in dec_files:
                    ...
                    if 'func0' in dec_exps and 'func0' in src_exps:
                        dec_exp_str = json.dump(json.load(dec_exps['func0']))
                        src_exp_str = json.dump(json.load(src_exps['func0']))
            ...
            for decompiler in decompilers:
                ...
                for dec_file in dec_files:
                    ...
                    if 'func_1' in dec_exps and 'func_1' in src_exps:
                        dec_exp_str = json.dump(json.load(dec_exps['func_1']))
                        src_exp_str = json.dump(json.load(src_exps['func_1']))
                        log_line = f"{dec_file}\t{dec_exp_str}\t{src_exp_str}"
...
def process_poj(dec_dir, src_dir, log_dir):
    compilers = ['clang', 'gcc']
    decompilers = ['angr', 'BinaryNinja', 'Ghidra', 'Hex-Rays', 'RetDec']
    optimizations = ['o0', 'o1', 'o2', 'o3', 'os']
    for compiler in compilers:
        for opt_level in optimizations:
            ...
            for decompiler in decompilers:
                ...
                for dec_file in dec_files:
                    ...
                    dec_exps = get_c_exp_len(dec_path, [])
                    src_exps = get_c_exp_len(src_path, [])

                    for func in dec_exps:
                        if func in src_exps:
                            dec_exp_str = json.dump(json.load(dec_exps[func]))
                            src_exp_str = json.dump(json.load(src_exps[func]))
