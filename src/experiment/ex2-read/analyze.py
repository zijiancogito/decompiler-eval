...
def func_cyclomatic_complexity_of_different_decompilers(root_dir, save_dir, compilers, optimizations, decompilers, colors):
    for compiler in compilers:
        for opt_level in optimizations:
            ...
            save_sub_dir = os.path.join(save_dir, 'cycle', compiler, opt_level)
...
def func_cyclomatic_complexity_of_different_optimizations(root_dir, save_dir, compilers, optimizations, decompilers, colors):
    for decompiler in decompilers:
        for compiler in compilers:
            csvs = {}

            for opt_level in optimizations:
                log_path = os.path.join(root_dir, compiler, opt_level, f"cycle-{decompiler}.csv")
                if not os.path.exists(log_path):
                    raise FileNotFoundError
                csvs[opt_level] = data_load.read_from_cycle(log_path)
            ...
            save_sub_dir = os.path.join(save_dir, 'cycle', decompiler, compiler)
            if not os.path.exists(save_sub_dir):
                os.makedirs(save_sub_dir)
...
def func_loc_of_different_decompilers(root_dir, save_dir, compilers, decompilers, optimizations, colors):
    for compiler in compilers:
        for opt_level in optimizations:
...
def func_var_of_different_decompilers(root_dir, save_dir, compilers, optimizations, decompilers, colors):
    for compiler in compilers:
        for opt_level in optimizations:
            log_dir = os.path.join(root_dir, compiler, opt_level)
            if not os.path.exists(log_dir):
...
def func_loc_of_different_optimizations(root_dir, save_dir, compilers, optimizations, decompilers, colors):
    for decompiler in decompilers:
        for compiler in compilers:
            csvs = {}
            ...
            save_sub_dir = os.path.join(save_dir, 'loc', decompiler, compiler)
            if not os.path.exists(save_sub_dir):
                os.makedirs(save_sub_dir)
...
def func_var_of_different_optimizations(root_dir, save_dir, compilers, optimizations, decompilers, colors):
    for decompiler in decompilers:
        for compiler in compilers:
            csvs = {}
            ...
            save_sub_dir = os.path.join(save_dir, 'var', decompiler, compiler)
            if not os.path.exists(save_sub_dir):
                os.makedirs(save_sub_dir)
...
if __name__ == '__main__':
    ...
    if args.experiment == 'decompilers':
        ...
        func_cyclomatic_complexity_of_different_optimizations(args.log, args.save)
