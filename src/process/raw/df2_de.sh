...
python3 check_err.py -o print -D df2 -l $ERR_LOG_DIR
    ...
    do
        ...
        do
            ...
            do
                ...
                python3 ./decompiling/preprocess/extractFunc/extractFunc.py -s $data_path -o $save_to -e $err_file -f func0 >/dev/null
                # echo Over: $data_path
            done
        done
    done
...
python3 remove_not_in_ir.py -d $DE_DIR -i $IR_DIR -D df2
...
python3 ./decompiling/preprocess/extractFunc/check_null_func.py -D df2 -d $DE_DIR -l $NULL_FUNC_LOG_DIR

echo "--------------------------------------------------------------------\n"
echo "Check _chk() function in decompiling results"
python3 remove_chk.py -i $DE_DIR -d df2 -o check
...
python3 check_if.py -i $DE_DIR -o check
...
rm -r $IF_MOVE_DIR
mkdir -p $IF_MOVE_DIR
python3 check_if.py -i $DE_DIR -o replace -l $IF_MOVE_DIR
...
python3 check_cond.py -o check -i $DE_DIR
...
python3 check_cond.py -o replace -i $DE_DIR -l $COND_MOVE_DIR
...
python3 check_concat.py -d df2 -i $DE_DIR_1 -l $CONCAT_LOG_DIR_1 -o replace
...
python3 check_concat.py -d df2 -i $DE_DIR_2 -l $CONCAT_LOG_DIR_2 -o replac
