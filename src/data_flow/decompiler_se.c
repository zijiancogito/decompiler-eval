...
Variable *variable_copy(Variable *v)
{
    ...
    if (v->type != NULL)
    {
        new_v->type = (char*)malloc(strlen(v->type));
        strcpy(new_v->type, v->type);
    }
    if (v->name != NULL)
    {
        new_v->name = (char*)malloc(strlen(v->name));
        strcpy(new_v->name, v->name);
    }
    if (v->value != NULL)
    {
        new_v->value = (char*)malloc(strlen(v->value));
...
void find_input_variables(TSTree *tree, const char *source, Variable **var_map)
{
    ...
    for (int i = 0; i < all_nodes.listLen; i++) {
        ...
        char *content = NULL;
        content = get_content_in_source(func_node, source);
        ...
        if (strstr(content, "scanf") != NULL)
        {
            ...
            {
                tmp = tmp->next;
                char *name = get_content_in_source(tmp->data, source);
                Variable *v = NULL;
...
TSNode find_branch_condition(TSNode node, const char* source)
{
    ...
    // printf("%s\n", ts_node_type(result));
    // printf("%s\n", get_content_in_source(result, source));
    return result;
}
...
char *parse_expression(TSNode expression_node, const char* source, Variable **var_map)
{
    ...
    {
        char *id_name = get_content_in_source(expression_node, source);
        Variable *v = NULL;
        HASH_FIND_STR(*var_map, id_name, v);
        if (v == NULL)
        {
            Variable *new_v;
            new_v = (Variable *)malloc(sizeof(*new_v));
            new_v->type = NULL;
            new_v->name = id_name;
            new_v->value = NULL;
        ...
        if (v->value != NULL)
        {
            cnt = (char*)malloc(sizeof(char) * strlen(v->value));
            strcpy(cnt, v->value);
            return cnt;
        }
        else
        {
            cnt = (char*)malloc(sizeof(char) * strlen(v->name));
            strcpy(cnt, v->name);
            return cnt;
        }
    }
    ...
    {
        return get_content_in_source(expression_node, source);
    }
    ...
    else if (strcmp("pointer_expression", node_type) == 0)
    {
        ...
        char *full_cnt = (char*)malloc(sizeof(char) * (strlen("*") + strlen(cnt)));
        strcpy(full_cnt, "*");
        strcat(full_cnt, cnt);
        // free(cnt);
        return full_cnt;
    ...
    else if (strcmp("parenthesized_expression", node_type) == 0)
    {
        ...
        char *cnt = parse_expression(cnt_node, source, var_map);
        char *full_cnt = (char*)malloc(sizeof(char) * (strlen("(") + strlen(cnt) + strlen(")")));
        strcpy(full_cnt, "(");
        strcat(full_cnt, cnt);
        strcat(full_cnt, ")");
        // free(cnt);
    ...
    else if (strcmp("binary_expression", node_type) == 0)
    {
        ...
        char *full_cnt = (char*)malloc(sizeof(char) * (strlen(cnt1) + strlen(cnt2) + strlen(cnt3)));
    ...
    else if (strcmp("update_expression", node_type) == 0)
    {
        ...
        if (strcmp(ts_node_type(second_node), "++") == 0 || strcmp(ts_node_type(second_node), "--") == 0)
        {
            ...
            full_cnt = get_content_in_source(first_node, source);
        }
        else
        {
            const char *op = ts_node_type(first_node);
            // cnt = parse_expression(second_node, source, var_map);
            cnt = get_content_in_source(second_node, source);
            char *single_op = (char*)malloc(strlen(op));
            strcpy(single_op, op);
            single_op[strlen(op) - 1] = '\0';
            char *full_cnt = (char*)malloc(strlen(cnt) + strlen(single_op) + strlen("1"));
            strcpy(full_cnt, cnt);
    ...
    {
        char *func_name = get_content_in_source(ts_node_child(expression_node, 0), source);
        ...
        char *func = (char*)malloc(sizeof(char) * length);
    ...
    else if (strcmp("return_statement", node_type) == 0)
    {
        ...
        if (return_cnt == NULL)
            return NULL;
        char *full_cnt = (char*)malloc(strlen("return ") + strlen(return_cnt));
...
void parse_assignment_expression(TSNode assign_node, const char* source, Variable **var_map, Variable_list *changed_vars)
{
    ...
    if (strcmp(ts_node_type(assign_node), "update_expression") == 0)
    {
        ...
        char *cnt = get_content_in_source(var_node, source);
        char *single_op = (char*)malloc(strlen(op));
        strcpy(single_op, op);
        single_op[strlen(op) - 1] = '\0';
        value = (char*)malloc(strlen(cnt) + strlen(single_op) + strlen("1"));
        strcpy(value, cnt);
    ...
    char *var_name = get_content_in_source(var_node, source);
    Variable *v = NULL;
    ...
    if (strcmp(ts_node_type(assign_node), "update_expression") != 0)
    {
        ...
        if (belong_to(op, self_ops, 12))
        {
            ...
            strcpy(single_op, op);
            single_op[strlen(op) - 1] = '\0';
            append = (char*)malloc(strlen(var_name) + strlen(single_op));
    ...
    if (append == NULL)
    {
        v->value = (char*)malloc(strlen(value));
        strcpy(v->value, value);
    }
    else
    {
        v->value = (char*)malloc(strlen(append) + strlen(value));
...
void parse_branch_condition(TSNode branch_node, const char* source, Variable **var_map, condition_list *true_conditions, condition_list *false_conditions)
{
    ...
    if (strcmp(node_type, "binary_expression") == 0)
    {
        ...
        if (belong_to(op_type, relational_ops, 6))
        {
            ...
            char *true_cnt = (char*)malloc(strlen(left_cnt) + strlen(op_type) + strlen(right_cnt));
            char *false_cnt = (char*)malloc(strlen(left_cnt) + strlen(opposite_op_type) + strlen(right_cnt));
        ...
        else if (strcmp(op_type, "||") == 0 || strcmp(op_type, "&&") == 0)
        {
            ...
            if (strcmp(op_type, "||") == 0)
            {
                for (int i = 0; i < left_true_con.len; i ++ )
                {
                    ...
                    char *con = (char*)malloc(strlen(left_true->condition));
                ...
                for (int i = 0; i < left_false_con.len; i ++ )
                {
                    ...
                    for (int j = 0; j < right_true_con.len; j ++ )
                    {
                        ...
                        char *con = (char*)malloc(strlen(left_false->condition) + strlen(", ") + strlen(right_true->condition));
                    ...
                    for (int j = 0; j < right_false_con.len; j ++)
                    {
                        ...
                        char *con = (char*)malloc(strlen(left_false->condition) + strlen(", ") + strlen(right_false->condition));
                        strcpy(con, left_false->condition);
                        strcat(con, ", ");
            ...
            else
            {
                ...
                {
                    left_false = left_false->next;
                    char *con = (char*)malloc(strlen(left_false->condition));
                    strcpy(con, left_false->condition);
                    append_condition(false_conditions, con);
                }
                for (int i = 0; i < left_true_con.len; i ++ )
                {
                    ...
                    {
                        right_false = right_false->next;
                        char *con = (char*)malloc(strlen(left_true->condition) + strlen(", ") + strlen(right_false->condition));
                        strcpy(con, left_true->condition);
                        strcat(con, ", ");
                        strcat(con, right_false->condition);
                        append_condition(false_conditions, con);
                    ...
                    {
                        right_true = right_true->next;
                        char *con = (char*)malloc(strlen(left_true->condition) + strlen(", ") + strlen(right_true->condition));
                        strcpy(con, left_true->condition);
                        strcat(con, ", ");
                        strcat(con, right_true->condition);
                        append_condition(true_conditions, con);
    ...
    else if (strcmp(node_type, "unary_expression") == 0)
    {
        ...
        if (strcmp(ts_node_type(unary_node), "!") == 0)
        {
            ...
            char *true_cnt = (char*)malloc(strlen(cnt) + strlen("==0"));
            char *false_cnt = (char*)malloc(strlen(cnt) + strlen("!=0"));
...
void print(path_condition *path, output_list *out_list, Variable *var_map)
{
    ...
    for (int i = 0; i < path->len; i ++ )
    {
        ...
        for (int j = 0; j < n; j ++ )
        {
            char *tmp_path = (char*)malloc(strlen(pathes[j]));
        ...
        for (int j = 0; j < tmp->len; j ++ )
        {
            ...
            for (int k = 0; k < n; k ++ )
            {
                char *p = (char*)malloc(strlen(tmp_pathes[k]) + strlen(", ") + strlen(tmp2->condition));
            ...
            if (n == 0)
            {
                char *p = (char*)malloc(strlen(tmp2->condition));
...
void symbolic_execution(Branch_Node **branch_map, Branch_Node * root, Variable **var_map, path_condition *path, output_list *out_list, NodeList *ignore_nodes, const char *source)
{
    ...
    while (tmp->next != all_nodes.tail)
    {
        ...
        {
            TSNode func_node = ts_node_child(tmp->data, 0);
            char *cnt = get_content_in_source(func_node, source);
        ...
        else if (is_branch_statement(node_type))
        {
            ...
            if (root->end == NULL)
            {
                ...
                if (branch != NULL)
                {
                    ...
                    symbolic_execution(branch_map, branch->false_branch, var_map, path, out_list, ignore_nodes, source);
                    delete_tail_condition_list(path);
                    append_condition_list(path, true_con);
                    symbolic_execution(branch_map, branch->true_branch, var_map, path, out_list, ignore_nodes, source);
                    delete_tail_condition_list(path);
                    delete_output_by_num(out_list, output_num);
                    update_var_map_with_var_list(var_map, changed_vars);
                    return ;
    ...
    {
        append_condition_list(path, false_con);
        symbolic_execution(branch_map, root->false_branch, var_map, path, out_list, ignore_nodes, source);
        delete_tail_condition_list(path);
        append_condition_list(path, true_con);
        symbolic_execution(branch_map, root->true_branch, var_map, path, out_list, ignore_nodes, source);
        delete_tail_condition_list(path);
    }
    delete_output_by_num(out_list, output_num);
    update_var_map_with_var_list(var_map, changed_vars);
...
void process(enum Decompiler decompiler, const char * filename)
{
    ...
    for (int i = 0; i < all_nodes.listLen; i++) {
        ...
        content = get_content_in_source(tmp->data, source);
        //printf("%s\n", ts_node_string(tmp->data));
        printf("%s\n", content);
        printf("\n");
        free(content);
