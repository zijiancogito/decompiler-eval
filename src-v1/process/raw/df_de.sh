...
python3 ./decompile/preprocess/extractFunc/check_null_func.py -t df2 -d $DE_DIR -l $NULL_FUNC_LOG_DIR -O $OPTIMIZATIONS -D $DECOMPILERS -C $COMPILERS
...
python3 ./decompile/preprocess/check_if/check_if.py -i $DE_DIR -o check -O $OPTIMIZATIONS -D $DECOMPILERS -C $COMPILERS
...
rm -r $IF_MOVE_DIR
mkdir -p $IF_MOVE_DIR
python3 ./decompile/preprocess/check_if/check_if.py -i $DE_DIR -o replace -l $IF_MOVE_DIR -O $OPTIMIZATIONS -D $DECOMPILERS -C $COMPILERS
...
python3 ./decompile/preprocess/check_if/check_cond.py -o check -i $DE_DIR -O $OPTIMIZATIONS -D $DECOMPILERS -C $COMPILERS
...
python3 ./decompile/preprocess/check_if/check_cond.py -o replace -i $DE_DIR -l $COND_MOVE_DIR -O $OPTIMIZATIONS -D $DECOMPILERS -C $COMPILERS
