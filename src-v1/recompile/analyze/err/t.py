                ...
                for k in ddd.keys():
                    if 'TOK (aka TOK) and TOK are' in k:
                        if 'TOK and TOK' not in dd:
                            ...
                            dd['TOK and TOK'] = dd['TOK and TOK'].union(ddd[k])
                        ...
                        if 'assigning to TOK' not in dd:
                            ...
                            dd['assigning to TOK'] = dd['assigning to TOK'].union(ddd[k])
                    elif 'called object' in k:
                        if 'called object' not in dd:
                            dd['called object'] = ddd[k]
                        else:
                            dd['called object'] = dd['called object'].union(ddd[k])
                    elif 'cannot assign' in k:
                        if 'cannot assign' not in dd:
                            dd['cannot assign'] = ddd[k]
                        else:
                            dd['cannot assign'] = dd['cannot assign'].union(ddd[k])
                    elif 'conflicting types' in k:
                        if 'conflicting types' not in dd:
                            dd['conflicting types'] = ddd[k]
                        else:
                            dd['conflicting types'] = dd['conflicting types'].union(ddd[k])
                    elif k.startswith('expected TOK') or k.startswith('expected expression') or k.startswith('expected statement') or k.startswith('expected identifier or TOK'):
                        print(k)
